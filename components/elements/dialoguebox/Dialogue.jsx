  ...
  responseParsed = responseString.split(/\r?\n/).map((line, index) => {
    ...
    useEffect(() => {
        if (!!userInput) {
            ...
            return () => {clearTimeout(delayedUnshowLogoSuggestion)}
        } else {
            setIsUserTyping(false);
        }
    }, [userInput]);
    ...
    return (
        <div className={style["dialogue-wrapper"]} id={isLoaded ? style["dialogue-wrapper-loaded"] : ''} data-testid="dialogue-wrapper">
